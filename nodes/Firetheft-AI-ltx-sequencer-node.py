...
class FiretheftLTXSequencer(io.ComfyNode):
    ...
    def define_schema(cls):
        ...
        # Flattened options to ensure link stability
        settings_options = []
        for n in range(1, 51):
            f_widgets = []
            for i in range(1, n + 1):
                f_widgets.append(io.Int.Input(f"frame_{i}", default=0, min=-9999, max=9999, step=1, display_name=f"Img #{i} Frame"))
                f_widgets.append(io.Float.Input(f"strength_{i}", default=1.0, min=0.0, max=1.0, step=0.001, display_name=f"Img #{i} Strength"))
            settings_options.append(io.DynamicCombo.Option(f"{n:02} | Frames", f_widgets))
            
            s_widgets = []
            for i in range(1, n + 1):
                s_widgets.append(io.Float.Input(f"second_{i}", default=0.0, min=0.0, max=9999.0, step=0.1, display_name=f"Img #{i} Second"))
                s_widgets.append(io.Float.Input(f"strength_{i}", default=1.0, min=0.0, max=1.0, step=0.001, display_name=f"Img #{i} Strength"))
    ...
    def execute(cls, positive, negative, vae, latent, multi_input, frame_rate, sequencer_settings, **kwargs) -> io.NodeOutput:
        ...
        for i in range(1, num_images + 1):
            ...
            if insert_mode == "frames":
                f_idx = sequencer_settings.get(f"frame_{i}", 0)
            elif insert_mode == "seconds":
                sec = sequencer_settings.get(f"second_{i}", 0.0)
                f_idx = int(sec * frame_rate)

            # Pre-clamp f_idx to prevent out-of-bounds math
            f_idx = max(0, f_idx)
